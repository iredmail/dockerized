...
RUN apk add --no-cache --progress \
        wget nginx php7 php7-fpm postfix postfix-pcre postfix-mysql dovecot dovecot-lmtpd dovecot-pop3d dovecot-pigeonhole-plugin dovecot-mysql clamav py-sqlalchemy py-setuptools py-dnspython py-mysqldb py-psycopg2 py2-pip mlmmj altermime py-requests uwsgi uwsgi-python uwsgi-syslog && \
